...
def download_cun_works_data(_works_data, _works_id, _users_id):
    ...
    if len(_works_data_img) > 0:
        print(
            f'========== ✅图组数据下载开始!   users_id: {_users_id} | works_id: {_works_id} ｜ works_title: {_works_title}')
        ...
        for index, _i_works_data in enumerate(_works_data_list):
            ...
            # 生成新文件名
            _new_file_name = f'{_works_id}_{_new_file_index + index + 1}.{_works_suffix}'
            # 处理新名字
            ...
            if _cun_status == 'exist':
                print(
                    f'========== 跳过下载,文件组已存在! users_id: {_users_id} | works_id: {_works_id} ｜ works_title: {_works_title}')
    return _cun_status
